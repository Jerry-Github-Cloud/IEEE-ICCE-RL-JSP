...
class JSP_Instance:
    ...
    def available_ops(self):
        ...
        avai_ops = []
        no_op_avai_ops = []
        #block_list = [1 for _ in self.machines]
        for m in self.machines:
            #min_waiting_time = self.args.max_process_time // 2
            min_waiting_time = -1
            #print(self.jobs)
            #print(self.current_time + min_waiting_time)
            for job in self.jobs:
                #print(job.job_id, job.current_op().avai_time )
                #if job.next_op() != -1:
                #    print({'m_id': m.machine_id, 'job_id': job.job_id, 'op_id': job.current_op_id, 'node_id': job.current_op().node_id})
                if job.done() == False and job.current_op().avai_time <= self.current_time:
                    if m.avai_time() <= self.current_time and m.machine_id == job.current_op().machine_id:
                        ...
                        #print("ADD", self.current_time, {'m_id': m.machine_id, 'job_id': job.job_id, 'op_id': job.current_op_id, 'node_id': job.current_op().node_id})
                        if min_waiting_time == -1 or job.current_op().process_time < min_waiting_time:
                            min_waiting_time = job.current_op().process_time
                #if job.done() == True and job.operations[job.next_op()].done == False and job.operations[job.next_op()].avai_time < self.current_time + min_waiting_time:
                #    if m.avai_time() < self.current_time + min_waiting_time and m.machine_id == job.operations[job.next_op()].machine_id:
                #        avai_ops.append({'m_id': m.machine_id, 'job_id': job.job_id, 'op_id': job.current_op_id, 'node_id': job.current_op().node_id})
                #elif job.done() == False and min_waiting_time != -1 and job.operations[job.next_op()].avai_time < self.current_time + min_waiting_time:
                elif job.done() == False and min_waiting_time != -1 and job.current_op().avai_time > self.current_time and job.current_op().avai_time < self.current_time + min_waiting_time:
                    #print(m.machine_id, job.current_op().avai_time , job.job_id, job.current_op_id)
                    if m.avai_time() < self.current_time + min_waiting_time and m.machine_id == job.current_op().machine_id and m.get_status(self.current_time) == 0:
                    #if m.avai_time() < self.current_time + min_waiting_time and m.machine_id == job.current_op().machine_id:
                        #print("ADD2", self.current_time, {'m_id': m.machine_id, 'job_id': job.job_id, 'op_id': job.current_op_id, 'node_id': job.current_op().node_id})
                        no_op_avai_ops.append({'m_id': m.machine_id, 'job_id': job.job_id, 'op_id': job.current_op_id, 'node_id': job.current_op().node_id})
            #for job in self.jobs:
        #for m in self.machines:
        #    for job in self.jobs:

        #print(self.current_time, m, avai_ops, no_op_avai_ops)
